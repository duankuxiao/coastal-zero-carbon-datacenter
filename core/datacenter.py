...
def _config_value(DC_Config, name, default):
    """Return an optional configuration value without requiring old configs to define it."""
    return getattr(DC_Config, name, default)


def calculate_seawater_cooling_power(CRAC_cooling_load, CRAC_Fan_load, CW_pump_load,
                                     seawater_temp, DC_Config):
    """Calculate cooling power for a seawater-source heat pump/free-cooling system.

    The model is intentionally parametric: it uses seawater temperature to choose
    between heat-exchanger free cooling and mechanical heat-pump operation, then
    adds seawater pumping and auxiliary heat-exchanger power. Values are in Watts
    unless otherwise noted.
    """
    seawater_temp = _config_value(DC_Config, "SEAWATER_DEFAULT_TEMP_C", 15.0) if seawater_temp is None else seawater_temp

    chilled_water_supply_temp = _config_value(DC_Config, "SEAWATER_CHILLED_WATER_SUPPLY_TEMP_C", 12.0)
    heat_exchanger_approach = _config_value(DC_Config, "SEAWATER_HEAT_EXCHANGER_APPROACH_C", 2.0)
    heat_pump_approach = _config_value(DC_Config, "SEAWATER_HEAT_PUMP_APPROACH_C", 5.0)
    evaporator_approach = _config_value(DC_Config, "SEAWATER_EVAPORATOR_APPROACH_C", 2.0)
    min_lift = _config_value(DC_Config, "SEAWATER_MIN_TEMP_LIFT_C", 3.0)
    carnot_efficiency = _config_value(DC_Config, "SEAWATER_COP_CARNOT_EFFICIENCY", 0.45)
    min_cop = _config_value(DC_Config, "SEAWATER_MIN_COP", 2.5)
    max_cop = _config_value(DC_Config, "SEAWATER_MAX_COP", 12.0)
    seawater_delta_t = max(_config_value(DC_Config, "SEAWATER_DELTA_T_C", 5.0), 0.1)
    seawater_pressure_drop = _config_value(DC_Config, "SEAWATER_PUMP_PRESSURE_DROP_PA", 250000.0)
    seawater_pump_efficiency = max(_config_value(DC_Config, "SEAWATER_PUMP_EFFICIENCY", 0.80), 0.01)
    seawater_cp = _config_value(DC_Config, "SEAWATER_CP_J_PER_KG_K", 3990.0)
    seawater_density = _config_value(DC_Config, "SEAWATER_DENSITY_KG_PER_M3", 1025.0)
    aux_power_ratio = _config_value(DC_Config, "SEAWATER_AUX_POWER_RATIO", 0.01)

    free_cooling_available = seawater_temp + heat_exchanger_approach <= chilled_water_supply_temp
    mode = "seawater_free_cooling" if free_cooling_available else "seawater_heat_pump"

    if CRAC_cooling_load <= 0:
        compressor_power = 0.0
        cop = float("inf")
        heat_rejection = 0.0
    elif free_cooling_available:
        compressor_power = 0.0
        cop = float("inf")
        heat_rejection = CRAC_cooling_load
    else:
        evap_temp_k = chilled_water_supply_temp - evaporator_approach + 273.15
        condenser_temp_k = seawater_temp + heat_pump_approach + 273.15
        temp_lift = max(condenser_temp_k - evap_temp_k, min_lift)
        ideal_cop = evap_temp_k / temp_lift
        cop = float(np.clip(carnot_efficiency * ideal_cop, min_cop, max_cop))
        compressor_power = CRAC_cooling_load / cop
        heat_rejection = CRAC_cooling_load + compressor_power

    seawater_flow_rate = heat_rejection / (seawater_density * seawater_cp * seawater_delta_t)
    seawater_pump_power = seawater_pressure_drop * seawater_flow_rate / seawater_pump_efficiency
    seawater_aux_power = aux_power_ratio * CRAC_cooling_load
    total_power = CRAC_Fan_load + CW_pump_load + compressor_power + seawater_pump_power + seawater_aux_power

    return {
        "mode": mode,
        "seawater_temp": seawater_temp,
        "free_cooling_active": free_cooling_available,
        "cooling_cop": cop,
        "compressor_power": compressor_power,
        "seawater_pump_power": seawater_pump_power,
        "seawater_aux_power": seawater_aux_power,
        "seawater_flow_rate_m3_s": seawater_flow_rate,
        "total_power": total_power,
    }


def calculate_HVAC_power_detailed(CRAC_setpoint, avg_CRAC_return_temp, ambient_temp,
                                  data_center_full_load, DC_Config, ctafr=None,
                                  seawater_temp=None):
    """Calculate HVAC power with detailed conventional and optional seawater components."""
    conventional = calculate_HVAC_power(
        CRAC_setpoint=CRAC_setpoint,
        avg_CRAC_return_temp=avg_CRAC_return_temp,
        ambient_temp=ambient_temp,
        data_center_full_load=data_center_full_load,
        DC_Config=DC_Config,
        ctafr=ctafr,
    )
    CRAC_Fan_load, CT_Fan_pwr, CRAC_cooling_load, chiller_power, CW_pump_load, CT_pump_load = conventional

    conventional_legacy_power = CT_Fan_pwr + chiller_power
    conventional_full_power = (
        CRAC_Fan_load + CT_Fan_pwr + chiller_power + CW_pump_load + CT_pump_load
    )

    cooling_mode = str(_config_value(DC_Config, "COOLING_SYSTEM_MODE", "conventional")).lower()
    selected_mode = "conventional"
    selected_power = conventional_legacy_power
    seawater = {
        "mode": "not_used",
        "seawater_temp": seawater_temp,
    ...
    if cooling_mode in ("conventional_full", "full_conventional"):
        selected_mode = "conventional_full"
        selected_power = conventional_full_power
    elif cooling_mode in ("seawater", "seawater_heat_pump", "seawater_hybrid", "seawater_free_cooling"):
        seawater = calculate_seawater_cooling_power(
            CRAC_cooling_load=CRAC_cooling_load,
            CRAC_Fan_load=CRAC_Fan_load,
            CW_pump_load=CW_pump_load,
            seawater_temp=seawater_temp,
            DC_Config=DC_Config,
        )
        selected_mode = seawater["mode"]
        selected_power = seawater["total_power"]

    return {
        "CRAC_Fan_load": CRAC_Fan_load,
        "CT_Fan_pwr": CT_Fan_pwr,
        "CRAC_cooling_load": CRAC_cooling_load,
        "Compressor_load": seawater["compressor_power"] if selected_mode.startswith("seawater") else chiller_power,
        "Conventional_Compressor_load": chiller_power,
        "CW_pump_load": CW_pump_load,
        "CT_pump_load": CT_pump_load,
        "selected_hvac_power": selected_power,
        "selected_cooling_mode": selected_mode,
        "conventional_legacy_hvac_power": conventional_legacy_power,
        "conventional_full_hvac_power": conventional_full_power,
        "seawater_hvac_power": seawater["total_power"],
        "seawater_temp": seawater["seawater_temp"],
        "seawater_free_cooling_active": seawater["free_cooling_active"],
        "seawater_cooling_cop": seawater["cooling_cop"],
        "seawater_pump_power": seawater["seawater_pump_power"],
        "seawater_aux_power": seawater["seawater_aux_power"],
        "seawater_flow_rate_m3_s": seawater["seawater_flow_rate_m3_s"],
        "cooling_power_savings": conventional_full_power - selected_power,
        "cooling_power_savings_pct": (
            (conventional_full_power - selected_power) / conventional_full_power
            if conventional_full_power > 0 else 0.0
        ),
    }

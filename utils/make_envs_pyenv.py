...
from core.bat_env_fwd_view import BatteryEnvFwd as battery_env_fwd
from core.carbon_ls import CarbonLoadEnv
from core.dc_gym import dc_gymenv
...
def make_dc_pyeplus_env(month : int = 1,
    ...
    if cooling_system_mode is not None:
        dc_config.COOLING_SYSTEM_MODE = cooling_system_mode
    ...
    legacy_hvac_ub = 1.1*ct_rated_load + 1.1*chiller_max_load
    full_hvac_extra_ub = (
        dc_config.CRAC_FAN_REF_P
        + (dc_config.CW_PRESSURE_DROP * dc_config.CW_WATER_FLOW_RATE) / dc_config.CW_PUMP_EFFICIENCY
        + (dc_config.CT_PRESSURE_DROP * dc_config.CT_WATER_FLOW_RATE) / dc_config.CT_PUMP_EFFICIENCY
    )
    hvac_range_ub = legacy_hvac_ub
    configured_cooling_mode = str(getattr(dc_config, "COOLING_SYSTEM_MODE", "conventional")).lower()
    if configured_cooling_mode.startswith("seawater") or configured_cooling_mode in ("conventional_full", "full_conventional"):
        hvac_range_ub = legacy_hvac_ub + full_hvac_extra_ub

    max_dc_power_w =  1.1*max(total_ite_pwr) + hvac_range_ub # Watts
    ...
    ranges = {
        ...
        'Facility Total HVAC Electricity Demand Rate(Whole Building)':  [0.0, hvac_range_ub],  # selected HVAC power
        'Facility Total Electricity Demand Rate(Whole Building)': [0.9*min(total_ite_pwr), 1.1*max(total_ite_pwr) + hvac_range_ub],  # TODO: This is not a part of the observation variables right now
